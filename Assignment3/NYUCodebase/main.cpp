...
void generateScene() {
	for (int i = 0; i < 7; i++) {
		for (int j = 0; j < 4; j++) {
			...
			invader->projectionMatrix = projectionMatrix;
			invader->viewMatrix = viewMatrix;
			invaders.push_back(invader);
		}
	}
	player = new Player(invadersTexture);
	player->position.y = -1.7f;
	player->projectionMatrix = projectionMatrix;
...
void Update() {
	...
	if (gameState == 0 && keys[SDL_SCANCODE_SPACE]) {
		...
		if (keys[SDL_SCANCODE_SPACE] && !spacekeyState && pbullets.size() < 10) {
			...
			pbullet->projectionMatrix = projectionMatrix;
			pbullet->viewMatrix = viewMatrix;
